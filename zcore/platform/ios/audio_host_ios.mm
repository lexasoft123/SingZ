...
constexpr uint32_t kRouteChanged = detail::IosAudioHostRouteChanged;
constexpr uint32_t kInterrupted = detail::IosAudioHostInterrupted;
constexpr uint32_t kMediaServicesLost = detail::IosAudioHostMediaServicesLost;
constexpr uint32_t kMediaServicesReset = detail::IosAudioHostMediaServicesReset;
...
detail::IosAudioHostPortKind portKind(AVAudioSessionPort port) {
  ...
  if ([port isEqualToString:AVAudioSessionPortCarAudio])
    return Kind::CarAudio;
  if ([port isEqualToString:AVAudioSessionPortHDMI])
    return Kind::Hdmi;
  return Kind::Unknown;
}
...
std::vector<std::string> channelLabels(AVAudioSessionPortDescription *port,
                                       uint32_t channels,
...
bool isPlanarFloat32(const AudioStreamBasicDescription &format, double rate,
                     uint32_t channels) noexcept {
  constexpr AudioFormatFlags required = kAudioFormatFlagIsFloat |
...
detail::IosAudioHostSessionSnapshot sessionSnapshot(
  ...
  result.routeGeneration =
      signals ? signals->routeGeneration.load(std::memory_order_acquire) : 0;
  ...
  result.inputKind =
      input ? portKind(input.portType) : detail::IosAudioHostPortKind::Unknown;
...
public:
  ...
  AudioHostInventory enumerate() const override {
    @autoreleasepool {
      ...
      if (snapshot.outputActive) {
        ...
        device.sampleRateRanges = {{snapshot.sampleRate, snapshot.sampleRate}};
  ...
  AudioHostResult open(const AudioHostConfig &config, AudioHostRender render,
    ...
    @autoreleasepool {
      stop();
      // A new open invocation is a new terminal-cause scope even when it
      // fails before RemoteIO reaches Open. Never attribute the previous
      // generation's retained route/interruption cause to this attempt.
      retainedTerminalCause_.reset();
      if (config.exclusive) {
      ...
      signals_->observerAdmission.open();
      ...
      osStatus = AudioUnitSetProperty(unit_, kAudioUnitProperty_StreamFormat,
      ...
      if (osStatus == noErr && hasInput) {
        osStatus = AudioUnitSetProperty(
            unit_, kAudioOutputUnitProperty_ChannelMap, kAudioUnitScope_Output,
            1, prepared.inputChannelMap.data(),
      ...
      if (osStatus != noErr) {
        return fail(
            ...
            osStatusMessage("set RemoteIO physical channel map", osStatus));
      }
      ...
      if (hasInput) {
        ...
        inputListStorage_.resize((listBytes + sizeof(std::max_align_t) - 1) /
        ...
        for (uint32_t channel = 0; channel < prepared.format.inputChannels;
          ...
          float *samples =
              inputSamples_.data() +
              static_cast<std::size_t>(channel) * prepared.format.maximumFrames;
      ...
      osStatus = AudioUnitGetProperty(unit_, kAudioUnitProperty_StreamFormat,
                                      kAudioUnitScope_Input, 0, &acceptedOutput,
                                      &propertySize);
      if (osStatus != noErr ||
      ...
      return {
          true, AudioHostError::None, AudioHostState::Open, format_, latency_,
          {}};
    }
  }
  ...
  void stop() noexcept override {
    @autoreleasepool {
      ...
      callback_->admission.beginClose();
      deactivateAudioHostCallback(&callback_->endpoint);
      detail::closeIosAudioHostSessionNotifications(signals_.get());
      removeObservers();
      detail::waitForIosAudioHostSessionNotifications(signals_.get());
      latchTerminalReason(false);
      ...
      while (callback_->endpoint.inFlight.load(std::memory_order_acquire) !=
             0) {
        std::this_thread::yield();
      }
      if (stopStatus != noErr) {
        latchTerminalReason(true);
        ...
        // The final admitted callback may have published its terminal cause
        // during the drains above. Retain it immediately before severing the
        // signals pointer.
        latchTerminalReason(false);
      ...
      if (providerFailure_.load(std::memory_order_acquire) != 0)
        latchTerminalReason(true);
      // Re-sample after callback admission/in-flight drain and after every
      // provider teardown call. No callback-visible signal may disappear at
      // the following reset without first reaching retained diagnostics.
      latchTerminalReason(false);
      retireRouteGeneration();
      unit_ = nullptr;
      initialized_ = false;
      ...
      if (disposeStatus == noErr) {
        ...
        signals_.reset();
        if (leaseHeld_)
          releaseIosAudioHostProcessLease(this);
      } else {
      ...
      // AudioOutputUnitStop proves callback admission closed, but a failed
      // AudioComponentInstanceDispose leaves a process-poisoning AudioUnit
      // owner in quarantine. Preserve a non-quiescent physical state so the
      // session retains its host marker and can never issue a fallback lease.
      state_.store(detail::iosAudioHostStateAfterDispose(
                       previous, disposeStatus == noErr),
  ...
  AudioHostStatus status() const noexcept override {
    ...
    const AudioHostTerminalCause retained = retainedTerminalCause_.current();
    const AudioHostTerminalCause live =
        signals_ ? signals_->firstTerminalCause.current()
                 : AudioHostTerminalCause{};
    const AudioHostTerminalCause terminal =
        firstAudioHostTerminalCause(retained, live);
    result.state = detail::iosAudioHostReportedState(
        state_.load(std::memory_order_acquire), terminal.reason);
    result.terminalReason = terminal.reason;
    result.terminalOrdinal = terminal.ordinal;
    // A retained cause is diagnostic and orthogonal to physical quiescence.
    // While the stream is live it makes the host terminal; after a proven
    // successful stop, status remains Stopped so owners can retire safely.
    result.format = format_;
    result.latency = latency_;
    result.routeGeneration =
        signals_ ? signals_->routeGeneration.load(std::memory_order_relaxed)
    ...
    result.callbacks =
        callback_->endpoint.callbacks.load(std::memory_order_relaxed);
...
private:
  ...
  void installObservers(
    ...
    auto notify = ^(uint32_t reason) {
      (void)detail::publishIosAudioHostSessionChange(signals.get(), reason);
    };
    routeObserver_ =
        [center addObserverForName:AVAudioSessionRouteChangeNotification
                        ...
                        usingBlock:^(NSNotification *) {
                          notify(kRouteChanged);
                        }];
    interruptionObserver_ =
  ...
  void removeObservers() noexcept {
    NSNotificationCenter *center = NSNotificationCenter.defaultCenter;
    if (routeObserver_)
      [center removeObserver:routeObserver_];
    if (interruptionObserver_)
      [center removeObserver:interruptionObserver_];
    if (mediaLostObserver_)
      [center removeObserver:mediaLostObserver_];
    if (mediaResetObserver_)
      [center removeObserver:mediaResetObserver_];
    routeObserver_ = nil;
  ...
  void recordProviderFailure(OSStatus status) noexcept {
    if (status == noErr)
      return;
    if (signals_ != nullptr) {
      signals_->firstTerminalCause.publish(
          AudioHostTerminalReason::ProviderFailure,
          AudioHostTerminalProducer::Provider);
    } else {
      retainedTerminalCause_.publish(AudioHostTerminalReason::ProviderFailure,
                                     AudioHostTerminalProducer::Provider);
    }
    int32_t expected = 0;
  ...
  void latchTerminalReason(bool providerFailure) noexcept {
    if (providerFailure) {
      if (signals_ != nullptr) {
        signals_->firstTerminalCause.publish(
            AudioHostTerminalReason::ProviderFailure,
    ...
    if (signals_ != nullptr)
      retainedTerminalCause_.retain(signals_->firstTerminalCause.current(),
                                    AudioHostTerminalProducer::ControlRetained);
  }

  AudioHostResult fail(AudioHostError error, std::string message) {
    // Map the live route/interruption/media-service fact before stop removes
    // observers and releases signals_. Provider failures without a platform
    // signal remain explicitly typed as provider failures.
    latchTerminalReason(error == AudioHostError::ProviderFailure);
    stop();
    state_.store(AudioHostState::Error, std::memory_order_release);
  ...
  AudioHostTerminalCauseLatch retainedTerminalCause_{};
