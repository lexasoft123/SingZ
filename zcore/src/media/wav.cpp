...
bool walkHeader(std::FILE* f, WavHeader& h) {
  ...
  for (;;) {
    ...
    if (std::memcmp(ch, "fmt ", 4) == 0) {
      ...
      haveFmt = true;
      if ((size & 1u) != 0 && fseeko(f, 1, SEEK_CUR) != 0) {
        h.error = "truncated fmt padding";
        return false;
      }
      ...
      const auto here = ftello(f);
      if (here < 0 || fseeko(f, 0, SEEK_END) != 0) {
        h.error = "cannot measure data chunk";
        return false;
      }
      const auto end = ftello(f);
      if (end < here || fseeko(f, here, SEEK_SET) != 0) {
        h.error = "cannot measure data chunk";
        return false;
      }
      const int64_t left = static_cast<int64_t>(end - here);
      ...
      const int64_t skip = static_cast<int64_t>(size) + (size & 1u);
      if (fseeko(f, skip, SEEK_CUR) != 0) {
        h.error = "truncated chunk";
        return false;
      }
    }
  }
