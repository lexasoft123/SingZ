...
7. Read every screenshot the drivers report (Read tool) and confirm it shows what the text claims. A blank window is a failure.
8. Every driver launches through `quiet-launch.cjs` (showInactive + dock.hide) and with `SINGZ_NO_SYNC=1` — never steal the singer's focus, never sync a driver's writes to the real Drive. Note the env var alone was NOT enough until main gated the dirty→scheduler wiring on it too: a save inside a driver marks the project dirty, and the scheduler would have pushed it four seconds later with the launch-sync gate green — code-certain, closed before it was ever observed, and verified by an align run that appended zero sync-log rows. A new driver copies a current driver's launch block, not an older one.
9. Flake note: the first wizard click can race the engine probe (driver retries internally); a selector timeout on `.lib-card` usually means a stale app instance survived — pkill and rerun.
