...
5. `node tests/e2e/mac/lyrics-song-switch-e2e.cjs` — the same race on the lyrics leg, made deterministic by delaying main's `net.fetch`; PASS means the next song keeps its own credit. Items 3 and 5 open a REAL library project and restore its files in a `finally` — if either is interrupted, check `git status` is not the place to look: verify the project.json of the song named in the output (the lyrics one also restores lyrics.json). The lyrics one refuses to run against a v1 project rather than restore a doc describing WAVs the migration deleted. (Item 4 works on a copy and has nothing of the singer's to put back.)
6. Read every screenshot the drivers report (Read tool) and confirm it shows what the text claims. A blank window is a failure.
7. Flake note: the first wizard click can race the engine probe (driver retries internally); a selector timeout on `.lib-card` usually means a stale app instance survived — pkill and rerun.
