...
10. `node_modules/.bin/electron tests/e2e/capture-addon-smoke.cjs [addon-path]` — loads the built capture addon inside real Electron (the ABI gate — a plain system-Node load proves nothing) and checks its exports and the `inputDevices`/`captureState`/`captureStats` result shapes. Opens no audio device, so it is safe in automated runs; CI runs it on macOS and Windows. Build the addon first (`npm run capture:addon`). Note the addon is a not-yet-wired transport: the shipped mic path does not exercise it, so this smoke is the only load coverage there is.
