...
| `playbackSwap` | `true` on both phones today | read, **not gated**: a `false` (an older binary) keeps native playback and makes every cue, training or pitch change a stop → prepare → start instead of a seam | — |
...
### The phones: sixteen, same name, same arity
...
| `suspendOutput` | 1 | generation — the background park's hold |
| `resumeOutput` | 1 | generation — its release |
...
`suspendOutput`/`resumeOutput` hold and release a parked song's stream while
the app is in the background. iOS never calls them (it keeps rendering in the
background by decision) and its host refuses them, but the surface is the same
on both bridges; on Android the release requires audio focus, like `start`.
...
`SingzCore`, not on this module, so it is outside the shared surface. Android
additionally exports `debugAudioFocusChange` (arity 1), a debug-build-only
door onto the focus listener that `focus-loss-android.cjs` drives; a release
build refuses it. Both are listed apart in the manifest (`iosOnly`,
`androidOnly`) so neither widens the shared list.

The TypeScript wrapper (`nativePlaybackBridge` in `native.ts`) requires ten of
the sixteen to exist and refuses the whole module otherwise. **Six are
...
resolves `null`, `unloadRetainingLanes()` falls back to `unload()`,
`positionNow()` falls back to the polled clock, and without
`suspendOutput()`/`resumeOutput()` a backgrounded park renders silence through
an open stream instead of holding it. In every case an old binary
...
There are 25 JNI symbols behind these, not 16: `nativePlaybackClaim` and
...
Twenty-five of the 44 `external fun` declarations in `split/SingzCore.kt` are
...
| `streamLanes` | bool | play the lanes straight out of their FLAC instead of decoding first (`config.streamLanes` in the core); absent = decode. A `parseBool` on iOS, a `jboolean` on Android, an optional bool config key on the desktop — the desktop bridge did not accept it until 0.21.1, which is why the desktop decoded six lanes for seconds on a Play the graph prepared ahead could not serve. The desktop sends it by default on macOS and Windows (`desktopStreamLanesPreferred`); Windows joined once the streaming source's `readAt` became a positioned `ReadFile` — the prepare dups a second descriptor per lane for the waveform pass, and seek+read on the shared file position was a race between the two threads | no |
...
transposeSemitones, countInAnchorSeconds?}` — seconds, a ratio, semitones, and
an optional anchor. `entrySeconds` is 0..43200; `playbackRate` 0.25..4.0;
`transposeSemitones` −24..24. `countInAnchorSeconds` (≥ `entrySeconds`; absent
or −1 = the entry) is where a count-in LANDS: the cue planner counts the
pre-roll down through negative frames to it, so a Play from a scrubbed or
...
83 keys on both phones plus three Android-only ones, polled every second
...
type. The other fourteen are the twelve read leniently and the two nothing
reads at all, both noted below.
...
`generation` already names its replacement; `retiringSwapGeneration` the one
being retired; `swapLandings`/`swapLateLandings` count the seams this stream
has landed and those that missed their budget; `swapPrimeNs` the last
...
`graphStatusCode`, `graphStatusDetail`, `timePitchAnchorOutcome` and the six
swap counters (`swapPendingGeneration`, `retiringSwapGeneration`,
`swapLandings`, `swapLateLandings`, `swapPrimeNs`, `swapLandingFrames`) default
...
| `AudioHostState` | 8 | `closed`, `open`, `running`, `stopped`, `device-lost`, `error`, `unsupported`, `suspended` (the background park's held stream: opened, not rendering, resumed in place) |
