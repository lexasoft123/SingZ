...
- `*_intrin_*.c` (avx2/sse/neon) — every one is guarded by the `FLAC__HAS_*INTRIN`
  ...
  has to be added to `third_party/native/CMakeLists.txt` and the iOS podspec
...
`fseek`. The root `SingZ::native_flac` wrapper passes it for host and Android.
The SingzCore podspec must do the same for its generated packaging copy.
