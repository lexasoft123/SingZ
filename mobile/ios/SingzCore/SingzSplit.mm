                  ...
                  rejecter:(RCTPromiseRejectBlock)reject)
{
  ...
  dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
    ...
    // same wrapper Android's JNI installs, for the same reason: the host
    // parity gate replays recorded logits, so it proves the pure logic and
    // says nothing about this file's marshalling, which is the one half that
    // only ever runs here. Wrapping the production callables means the dumped
    // tensors are the real path's, not a parallel one. Dead for every real
    // caller: the pipeline passes "".
