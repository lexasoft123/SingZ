...
| seek before Play, loop before Play | NOT driven by this scenario yet (its seeks and loops come after Play), which is how build 49 reached a phone with both refused by the core; `native-playback-b2` pins the facade's remembered seek/loop and Play's re-prepare, and the core's `countInLandsOnAnchorMidSong` pins the count-in that Play then plays from the scrubbed spot — a scenario leg for both is owed |
