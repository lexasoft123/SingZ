...
| seek before Play, loop before Play, the count-in from a scrubbed or paused spot | not driven by this scenario (its seeks and loops come after Play), which is how build 49 reached a phone with both refused by the core; `mobile/tests/play-from-anywhere.cjs` drives them now on both platforms, `native-playback-b2` pins the facade and the core's `countInLandsOnAnchorMidSong` the landing |
