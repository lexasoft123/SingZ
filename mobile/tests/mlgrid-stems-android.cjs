#!/usr/bin/env node
/**
 * Phase 4b, the pipeline's entry: the Beat This! grid FROM STEMS on device,
 * against a recording made from the same stems mixed on the host.
 *
 ...
 *
 *   ANDROID_SERIAL=emulator-5554 METRO_PORT=8082 [ANDROID_PKG=…] [DEVICE_NAME=…] \
 *     node mobile/tests/mlgrid-stems-android.cjs <recording-dir> <stems-dir>
 *
 ...
 * Preconditions: debug build, Metro, and both beat models in the device's
 * files/mlt (the same dir the sibling suite uses); this suite pushes the
 * stems itself into files/mlt/stems.
 */
// Every E2E driver runs under a deadline: a hang prints where it was and
// exits, instead of sitting there until somebody notices (tests/shared/watchdog.cjs).
require('../../tests/shared/watchdog.cjs').arm('mlgrid-stems-android')
...
if (!rec || !stemsDir) die('usage: node mobile/tests/mlgrid-stems-android.cjs <recording-dir> <stems-dir>')
...
;(async () => {
  ...
  const target = list.filter((x) => (x.deviceName || '').includes(DEV) && x.webSocketDebuggerUrl).pop()
  if (!target) die(`no Android target named '${DEV}' on Metro ${PORT} — is the app running?`)
  ...
  const paths = stems.map((f) => `${DIR}/stems/${f}`)
  ...
  console.log(bad === 0 ? 'MLGRID FROM STEMS: the phone hears the desktop\'s lattice' : `${bad} CHECK(S) DIVERGE`)
