...
describe('native playback bridge arity', () => {
  ...
  it('exports the same methods with the same arity on both platforms', () => {
    ...
    const androidShared = android.filter(
      method => !bridgeManifest.methods.androidOnly.some(only => only.name === method.name),
    );
    expect(shared).toEqual(androidShared);
  });
  ...
  // declared as such. codecTargetProof sits behind a compile flag (Android's
  // equivalent lives on SingzCore rather than on this module), and Android's
  // debugAudioFocusChange is the debug-build door focus-loss-android.cjs
  // drives through.
  it('declares every platform-specific method instead of letting it widen the surface', () => {
    ...
    const iosNames = new Set(ios.map(method => method.name));
    const iosOnly = ios.filter(method => !androidNames.has(method.name));
    const androidOnly = android.filter(method => !iosNames.has(method.name));
    expect(iosOnly).toEqual(bridgeManifest.methods.iosOnly);
    expect(androidOnly).toEqual(bridgeManifest.methods.androidOnly);
    expect(module).toMatch(/fun debugAudioFocusChange\([\s\S]*?if \(!BuildConfig\.DEBUG\)/);
