...
struct OboeCallbackBridge final : oboe::AudioStreamDataCallback,
  ...
  std::atomic<OboeInputBackend*> owner{nullptr};
...
class OboeInputBackend final : public AudioInputBackend {
 public:
  ...
  AudioInputResult open(const AudioInputConfig& config, AudioInputPush push,
    ...
    callbacks_->owner.store(this, std::memory_order_release);
    ...
    if (opened != oboe::Result::OK || !stream_) {
      callbacks_->owner.store(nullptr, std::memory_order_release);
      stream_.reset();
 ...
 private:
  ...
  void closeStream() {
    ...
    // while the stream is still open, before callback admission closes or
    // close() can invalidate the handle.
    stopTimestampSampler();
    callbackGate_.beginClose();
    auto stream = stream_;
    // Cleared unconditionally, above the early return: otherwise a teardown
    // with no stream leaves the bridge still pointing at a backend about to
    // die. Safe here because what protects a callback already inside the
    // backend is the inFlight() wait below, not where this store sits.
    callbacks_->owner.store(nullptr, std::memory_order_release);
    if (!stream) return;
    if (started_) (void)stream->requestStop();
    // The callback contains no blocking operation, so this wait is finite, and
    // it guarantees no callback is still touching mono_/push_ when they go.
    while (callbackGate_.inFlight() != 0)
      std::this_thread::sleep_for(std::chrono::milliseconds(1));
    // Oboe's error thread calls close() and then onErrorAfterClose; with the
    // back-pointer already cleared above, neither has anything of ours to
    // reach.
    (void)stream->close();
    stream_.reset();
    started_ = false;
...
oboe::DataCallbackResult OboeCallbackBridge::onAudioReady(oboe::AudioStream* /*stream*/,
  ...
  OboeInputBackend* backend = owner.load(std::memory_order_acquire);
...
void OboeCallbackBridge::onErrorBeforeClose(oboe::AudioStream* /*stream*/,
                                            oboe::Result error) {
  if (OboeInputBackend* backend = owner.load(std::memory_order_acquire))
    backend->onErrorBeforeClose(error);
}
