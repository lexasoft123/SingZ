 ...
 * of which runs here: the sources are compiled directly by the app CMakeLists
 * (Android) and the SingzCore podspec (iOS), so the handful of symbols those
 * builds actually depend on are stated once, here.
 *
 ...
 * scripts/run-core-host-tests.sh passes it; the Android CMakeLists and the
 * SingzCore podspec must too.
 */
