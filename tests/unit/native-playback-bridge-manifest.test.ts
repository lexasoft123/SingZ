...
describe('desktop playback status', () => {
  ...
  it('carries every phone session key, relocating only the four the manifest lists', () => {
    ...
    for (const key of manifest.session.common) {
      ...
      // Phone-only by design: the block boundary swap exists on the phones
      // alone today, so the addon emits none of its six counters. Declared,
      // not inferred — a seventh key the addon forgets is still a red here.
      if (manifest.session.desktop.absent.includes(key)) continue
...
describe('the phone session parser', () => {
  ...
  // 69 strict + 12 lenient + 2 unparsed = the 83 both phones emit (the six
  // swap counters are lenient: a seam is a facade-side decision, and a
  // binary without them must keep playing). Deleting one
