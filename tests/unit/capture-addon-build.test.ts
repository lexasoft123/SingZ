...
describe('Electron capture addon build', () => {
  ...
  it('packages and smoke-checks one coherent worktree-local snapshot', () => {
    ...
    expect(checks).toContain('> vendor/win32-x64/singz-analyze\n')
    expect(checks).not.toContain('whisper-cli')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      try {
        ...
        symlinkSync(source, join(engines, 'llama-server'))
        afterPack.materializeLinkedEngines(engines)
        const copied = join(engines, 'llama-server')
