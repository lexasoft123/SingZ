...
import { accentIndex, barNumber, beatIndexAtOrAfter, type BeatInfo } from '../audio/beat'
 ...
 */
export default function BeatGrid({ grid, viewS, viewE, rulerH, lanes }: Props): React.JSX.Element {
  ...
  const paint = useCallback((): void => {
    ...
    for (let i = Math.max(0, beatIndexAtOrAfter(g, viewS)); i < g.beats.length; i++) {
      ...
      if (down) {
        bar = barNumber(g, i)
        if (stride > 1 && (((bar - 1) % stride) + stride) % stride !== 0) continue
      }
      const x = snap(((t - viewS) / span) * w)
      ctx.fillStyle = down ? 'rgba(255, 160, 40, 0.5)' : 'rgba(255, 240, 214, 0.22)'
      ctx.fillRect(x, rulerH, down ? hair * 2 : hair, h - rulerH)
      if (down && x - lastLabelX >= MIN_LABEL_PX) {
        lastLabelX = x
  ...
  useEffect(paint, [paint, grid, viewS, viewE, rulerH])

  return (
    <canvas
      className="beat-lines"
      ref={canvasRef}
      style={{ gridRow: `1 / ${lanes + 2}` }}
      aria-hidden
    />
  )
}
